...
1:
    ...
    Private Sub DateTimePicker2_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles TEXTBasicWorkingHours.EditValueChanged, TimeAttendance.EditValueChanged, TimeToLeave.EditValueChanged
        Try
            Dim F As Double
            F = DateDiff(DateInterval.Minute, Me.TimeAttendance.EditValue, Me.TimeToLeave.EditValue) / 60
            Me.TEXTOvertimeHours.EditValue = (F - Val(Me.TEXTBasicWorkingHours.EditValue))
        Catch ex As Exception
            MessageBox.Show("حدث خطأ أثناء حساب ساعات العمل الإضافية: " & ex.Message)
        End Try
    End Sub
