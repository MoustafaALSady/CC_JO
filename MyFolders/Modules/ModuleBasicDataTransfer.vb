...
Friend Module ModuleBasicDataTransfer
    ...
    Public Sub BasicDataTransfer(ByVal XxFB As FinaBalances2)
        ...
        Try
            ...
            'حسابات الصندوق
            Dim strsql_CASHIER As New SqlClient.SqlCommand("", Consum) With {
                .CommandText = "SELECT CSH18, CSH19 FROM CASHIER  WHERE Year(CSH2) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'  and CUser ='" & CUser & "'GROUP BY CSH18, CSH19"
            }
            Dim ds_CASHIER As New DataSet
            Dim Adp_CASHIER As New SqlClient.SqlDataAdapter(strsql_CASHIER)
            ds_CASHIER.Clear()
            Adp_CASHIER.Fill(ds_CASHIER)
            'حسابات البنك 
            Dim strsql_BANKJO As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT  EBNK10, EBNK11 FROM BANKJO  WHERE  CUser ='" & CUser & "'and Year(EBNK3) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'GROUP BY EBNK10, EBNK11 "
                }
            Dim ds_BANKJO As New DataSet
            Dim Adp_BANKJO As New SqlClient.SqlDataAdapter(strsql_BANKJO)
            ds_BANKJO.Clear()
            Adp_BANKJO.Fill(ds_BANKJO)
            'حسابات الشيكات
            Dim strsql_Checks As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT IDCH FROM Checks  WHERE  CH17 = '" & False & "' and CUser ='" & CUser & "'"
                }
            Dim ds_Checks As New DataSet
            Dim Adp_Checks As New SqlClient.SqlDataAdapter(strsql_Checks)
            ds_Checks.Clear()
            Adp_Checks.Fill(ds_Checks)
            Dim IDCH As String = Nothing
            'حسابات الاعضاء
            Dim strsql_Deposits As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT TBNK4,TBNK5,TBNK6,TBNK7,TBNK8,TBNK9,TBNK10,TBNK11,TBNK13,TBNK15,TBNK15,TBNK16,TBNK19,TBNK20,TBNK21    FROM Deposits WHERE CUser ='" & CUser & "' and Year(TBNK3) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'"
                }
            Dim ds_Deposits As New DataSet
            Dim Adp_Deposits As New SqlClient.SqlDataAdapter(strsql_Deposits)
            ds_Deposits.Clear()
            Adp_Deposits.Fill(ds_Deposits)
            Dim sql_shares2 As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT TBNK22, TBNK23, TBNK2, TBNK3, TBNK4, TBNK5, OpeningBalance1, TBNK6, TBNK8, CUser  FROM shares2 WHERE    CUser ='" & CUser & "' and Year(TBNK3) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'"
                }
            Dim ds_Shares_2 As New DataSet
            Dim AdpShares_2 As New SqlClient.SqlDataAdapter(sql_shares2)
            ds_Shares_2.Clear()
            AdpShares_2.Fill(ds_Shares_2)
            Dim strsql_Shares_1 As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT TBNK23, TBNK22, TBNK2, TBNK3, TBNK6, TBNK8, TBNK4, TBNK5, OpeningBalance, CUser  FROM shares1 WHERE    CUser ='" & CUser & "' and Year(TBNK3) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'"
                }
            Dim ds_Shares_1 As New DataSet
            Dim AdpShares_1 As New SqlClient.SqlDataAdapter(strsql_Shares_1)
            ds_Shares_1.Clear()
            AdpShares_1.Fill(ds_Shares_1)
            'حسابات السحب والايداع 
            Dim strsql_shares2 As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT TBNK22, TBNK23, TBNK2, TBNK3, TBNK4, TBNK5, OpeningBalance1, TBNK6, TBNK8, CB1, BN2  , CUser  FROM shares2 WHERE    CUser ='" & CUser & "' and Year(TBNK3) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'"
                }
            Dim ds_shares2 As New DataSet
            Dim Adp_shares2 As New SqlClient.SqlDataAdapter(strsql_shares2)
            ds_shares2.Clear()
            Adp_shares2.Fill(ds_shares2)
            'حسابات السحب والايداع 
            Dim strsql_shares1 As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT TBNK23, TBNK22, TBNK2, TBNK3, TBNK6, TBNK8, TBNK4, TBNK5, OpeningBalance, CB1, BN2  , CUser  FROM shares1 WHERE    CUser ='" & CUser & "' and Year(TBNK3) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'"
                }
            Dim ds_shares1 As New DataSet
            Dim Adp_shares1 As New SqlClient.SqlDataAdapter(strsql_shares1)
            ds_shares1.Clear()
            Adp_shares1.Fill(ds_shares1)
            'حسابات العملاء
            Dim strsql_SumCABLES As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT IDCAB FROM SumCABLES   WHERE   CUser='" & CUser & "' and Year(CAB3) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'"
                }
            Dim ds_SumCABLES As New DataSet
            Dim Adp_SumCABLES As New SqlClient.SqlDataAdapter(strsql_SumCABLES)
            ds_SumCABLES.Clear()
            Adp_SumCABLES.Fill(ds_SumCABLES)
            'حسابات الموردين 
            Dim strsql_SumSuppliers As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT IDCAB FROM SumSuppliers  WHERE   CUser='" & CUser & "' and Year(CAB3) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'"
                }
            Dim ds_SumSuppliers As New DataSet
            Dim Adp_SumSuppliers As New SqlClient.SqlDataAdapter(strsql_SumSuppliers)
            ds_SumSuppliers.Clear()
            Adp_SumSuppliers.Fill(ds_SumSuppliers)
            'عهدة الموظفي
            Dim strsql_SumEMPSOLF As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT Sumcsh,CSH14 ,CSH15 FROM SumEMPSOLF  WHERE   CUser='" & CUser & "' and Year(CSH2) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'"
                }

            Dim ds_SumEMPSOLF As New DataSet
            Dim Adp_SumEMPSOLF As New SqlClient.SqlDataAdapter(strsql_SumEMPSOLF)
            ds_SumEMPSOLF.Clear()
            Adp_SumEMPSOLF.Fill(ds_SumEMPSOLF)
            'حسابات عهدة الذمم المدينة
            Dim strsql_CustomersCABLES As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT Lo FROM CustomersCABLES2 WHERE   CUser='" & CUser & "' and Year(lo2) ='" & FiscalYear_currentDateMustBeInFiscalYear() & "'"
                }
            Dim ds_CustomersCABLES As New DataSet
            Dim Adp_CustomersCABLES As New SqlClient.SqlDataAdapter(strsql_CustomersCABLES)
            ds_CustomersCABLES.Clear()
            Adp_CustomersCABLES.Fill(ds_CustomersCABLES)
            'حسابات عهدة الاصناف
            Dim strsqlSTOCKS As New SqlClient.SqlCommand("", Consum) With {
                    .CommandText = "SELECT STK1 FROM STOCKS WHERE   CUser='" & CUser & "' and STK13 >'" & 0 & "'"
                }
            Dim dsSTOCKS As New DataSet
            Dim AdpSTOCKS As New SqlClient.SqlDataAdapter(strsqlSTOCKS)
            dsSTOCKS.Clear()
            AdpSTOCKS.Fill(dsSTOCKS)

            Using ConTransaction As New SqlConnection(constring)
                If ConTransaction.State = ConnectionState.Closed Then ConTransaction.Open()
                ...
                Try
                    ...
                    For i As Integer = 0 To XxFB.DataGridView1.RowCount - 1
                        ...
                        Dim CMD As New SqlClient.SqlCommand(command.CommandText, ConTransaction, Trans)
                        With CMD.Parameters
                    ...
                    For Ix As Integer = 0 To ds_Checks.Tables(0).Rows.Count - 1
                        ...
                        command.CommandText = "UPDATE Checks SET CH2 = @CH2 WHERE IDCH = @IDCH"
                        Dim CMD As New SqlClient.SqlCommand(command.CommandText, ConTransaction, Trans)
                        CMD.Parameters.Clear()
                        CMD.Parameters.AddWithValue("@IDCH", IDCH)
                        CMD.Parameters.AddWithValue("@CH2", "1-1-" & FiscalYear_currentDateMustBeInFiscalYear() + 1)
                        CMD.CommandTimeout = 3600
                        CMD.ExecuteNonQuery()
                    Next
                    ''حسابات الاعضاء
                    ...
                    For Ix As Integer = 0 To ds_SumCABLES.Tables(0).Rows.Count - 1
                        ...
                        command.CommandText = "UPDATE CABLES SET CAB3 = @CAB3 WHERE IDCAB = @IDCAB"
                        Dim CMD As New SqlClient.SqlCommand(command.CommandText, ConTransaction, Trans)
                        CMD.Parameters.Clear()
                        CMD.Parameters.AddWithValue("@IDCAB", TEXTID)
                        CMD.Parameters.AddWithValue("@CAB3", "1-1-" & FiscalYear_currentDateMustBeInFiscalYear() + 1)
                        CMD.CommandTimeout = 3600
                        CMD.ExecuteNonQuery()
                    Next
                    ''حسابات الموردين 
                    For II As Integer = 0 To ds_SumSuppliers.Tables(0).Rows.Count - 1
                        ...
                        Else
                            IDCAB += 0
                        End If
                        command.CommandText = "UPDATE Suppliers1 SET CAB3 = @CAB3 WHERE IDCAB = @IDCAB"
                        Dim CMD As New SqlClient.SqlCommand(command.CommandText, ConTransaction, Trans)
                    ...
                    For IX As Integer = 0 To ds_CustomersCABLES.Tables(0).Rows.Count - 1
                        ...
                        Lo = ds_CustomersCABLES.Tables(0).Rows(IX).Item("Lo")
                        command.CommandText = "UPDATE Loans SET   lo2 = @lo2  WHERE lo = @lo"
                        Dim cmd As New SqlClient.SqlCommand(command.CommandText, ConTransaction, Trans)
                        cmd.Parameters.AddWithValue("@lo", Lo)
                        cmd.Parameters.AddWithValue("@lo2", "1-1-" & FiscalYear_currentDateMustBeInFiscalYear() + 1)
                        cmd.CommandTimeout = 3600
                        cmd.ExecuteNonQuery()
                    Next
                    'حسابات عهدة الاصناف
                    For II As Integer = 0 To dsSTOCKS.Tables(0).Rows.Count - 1
                        XxFB.Labelstatus.Text = "الرجاء الانتظار ـ- ترحيل حسابات عهدة الاصناف -ـ-ـ-ـ"
                        STK1 = dsSTOCKS.Tables(0).Rows(II).Item("STK1")
                        command.CommandText = "UPDATE STOCKS SET     STK4 = @STK4  WHERE STK1 = @STK1"
                        Dim CMD As New SqlClient.SqlCommand(command.CommandText, ConTransaction, Trans)
                        CMD.Parameters.AddWithValue("@STK1", STK1)
                        CMD.Parameters.AddWithValue("@STK4", "1-1-" & FiscalYear_currentDateMustBeInFiscalYear() + 1)
                        CMD.CommandTimeout = 3600
                        CMD.ExecuteNonQuery()
                    Next
